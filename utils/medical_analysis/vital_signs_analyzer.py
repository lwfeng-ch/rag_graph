...
from .medical_reference import medical_db, RiskLevel, ReferenceRange
...
class VitalSignsAnalyzer:
    ...
    def analyze_vital_signs(
        ...
        logger.info(
            f"生命体征分析完成: {abnormal_count} 个异常指标，风险等级: {overall_risk.value}"
        )
        return result

    def generate_diagnosis_hints(
        self, indicators: List[VitalSignIndicator]
    ) -> List[str]:
        """
        ...
        sbp = next(
            (i for i in indicators if i.name_en == "Systolic Blood Pressure"), None
        )
        dbp = next(
            (i for i in indicators if i.name_en == "Diastolic Blood Pressure"), None
        )
        ...
        if sbp and dbp:
            if sbp.status != "正常" or dbp.status != "正常":
                if sbp.value >= 140 or dbp.value >= 90:
                    hints.append(
                        "高血压（≥140/90 mmHg）→ 提示心血管风险，建议心内科就诊"
                    )
