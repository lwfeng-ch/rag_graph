...
class BiochemistryAnalyzer:
    ...
    def analyze_correlations(
        ...
        alt = next((i for i in indicators if "ALT" in i.name_en or "谷丙转氨酶" in i.name), None)
        ast = next((i for i in indicators if "AST" in i.name_en or "谷草转氨酶" in i.name), None)
        
        if glu and cr:
            if glu.status != "正常" and cr.status != "正常":
                if glu.value > glu.reference.normal_max and cr.value > cr.reference.normal_max:
                    correlations.append("血糖升高 + 肌酐升高 → 提示糖尿病肾病可能，建议内分泌科和肾内科就诊")
        
        if alt and ast:
            if alt.status != "正常" or ast.status != "正常":
                if alt.value > alt.reference.normal_max and ast.value > ast.reference.normal_max:
                    ast_alt_ratio = ast.value / alt.value if alt.value > 0 else 0
                    if ast_alt_ratio > 2:
                        correlations.append("AST/ALT > 2 → 提示可能存在酒精性肝病或肝硬化")
                    elif ast_alt_ratio < 1:
                        correlations.append("ALT > AST → 提示可能存在急性肝炎或脂肪肝")
        
        if glu and alt:
            if glu.status != "正常" and alt.status != "正常":
                if glu.value > glu.reference.normal_max and alt.value > alt.reference.normal_max:
                    correlations.append("血糖升高 + 肝功能异常 → 提示代谢综合征可能，建议综合评估")
    ...
    def generate_diagnosis_hints(
        ...
        alt = next((i for i in indicators if "ALT" in i.name_en or "谷丙转氨酶" in i.name), None)
        ast = next((i for i in indicators if "AST" in i.name_en or "谷草转氨酶" in i.name), None)
        hba1c = next((i for i in indicators if "Glycated Hemoglobin" in i.name_en), None)
        
        if glu and glu.status != "正常":
            if glu.value > glu.reference.normal_max:
                if glu.value >= 7.0:
                    hints.append("空腹血糖 ≥ 7.0 mmol/L → 提示糖尿病可能，建议内分泌科就诊")
                elif glu.value >= 6.1:
                    hints.append("空腹血糖 6.1-7.0 mmol/L → 提示糖尿病前期，建议控制饮食、加强运动")
        
        if hba1c and hba1c.status != "正常":
            if hba1c.value > 7.0:
                hints.append("糖化血红蛋白 > 7% → 提示近期血糖控制不佳，建议调整治疗方案")
    ...
    def generate_recommendations(
        ...
        if glu and glu.status != "正常" and glu.value > glu.reference.normal_max:
            recommendations.append("血糖异常建议：控制碳水化合物摄入，增加运动，定期监测血糖")
        
        cr = next((i for i in indicators if "Creatinine" in i.name_en), None)
        if cr and cr.status != "正常" and cr.value > cr.reference.normal_max:
            recommendations.append("肾功能异常建议：低盐低蛋白饮食，避免使用肾毒性药物，定期复查肾功能")
        
        alt = next((i for i in indicators if "ALT" in i.name_en or "谷丙转氨酶" in i.name), None)
        if alt and alt.status != "正常" and alt.value > alt.reference.normal_max:
            recommendations.append("肝功能异常建议：戒酒，避免高脂饮食，慎用肝毒性药物，定期复查肝功能")
    ...
    def analyze(
        ...
        logger.info(f"血生化分析完成: {abnormal_count} 个异常指标，风险等级: {overall_risk.value}")
        return result
    ...
    def _generate_summary(
        ...
        summary = f"血生化检查发现 {abnormal_count} 项异常指标：{', '.join(abnormal_names)}。"
