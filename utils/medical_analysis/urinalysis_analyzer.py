...
class UrinalysisAnalyzer:
    ...
    def analyze_qualitative_indicator(
        self, indicator_name: str, value: str
    ) -> UrinalysisIndicator:
        """
        分析定性指标。
        ...
        if not reference:
            ...
            return UrinalysisIndicator(
                name=indicator_name, name_en=indicator_name, value=value, status="未知"
            )
    ...
    def generate_diagnosis_hints(
        self, indicators: List[UrinalysisIndicator]
    ) -> List[str]:
        """
        基于异常指标生成诊断提示。
    ...
    def generate_recommendations(
        ...
        if pro and pro.status != "正常":
            recommendations.append(
                "尿蛋白异常建议：低盐低蛋白饮食，避免剧烈运动，定期复查尿常规"
            )

        glu = next((i for i in indicators if i.name_en == "Urine Glucose"), None)
        if glu and glu.status != "正常":
            recommendations.append(
                "尿糖异常建议：控制碳水化合物摄入，监测血糖，内分泌科就诊"
            )
    ...
    def analyze(self, report_text: str) -> UrinalysisAnalysisResult:
        ...
        logger.info(
            f"尿常规分析完成: {abnormal_count} 个异常指标，风险等级: {overall_risk.value}"
        )
        return result
    ...
    def _generate_summary(
        ...
        summary = (
            f"尿常规检查发现 {abnormal_count} 项异常指标：{', '.join(abnormal_names)}。"
        )
