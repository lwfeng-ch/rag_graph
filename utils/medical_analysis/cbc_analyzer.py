...
class CBCAnalyzer:
    ...
    def parse_cbc_report(self, report_text: str, gender: Gender = Gender.UNKNOWN) -> Dict[str, float]:
        """
        解析血常规报告文本，提取指标数值。
    ...
    def generate_diagnosis_hints(self, indicators: List[CBCIndicator]) -> List[str]:
        ...
        wbc = next((i for i in indicators if i.name_en == "White Blood Cell Count"), None)
    ...
    def generate_recommendations(self, indicators: List[CBCIndicator]) -> List[str]:
        ...
        medium_risk_indicators = [i for i in indicators if i.risk_level == RiskLevel.MEDIUM]
        ...
        if wbc and wbc.status != "正常":
            if wbc.value > wbc.reference.normal_max:
                recommendations.append("白细胞升高建议：注意休息，多饮水，如有发热症状及时就医")
    ...
    def analyze(
        ...
        logger.info(f"血常规分析完成: {abnormal_count} 个异常指标，风险等级: {overall_risk.value}")
        return result
    
    def _generate_summary(
        self,
        indicators: List[CBCIndicator],
        abnormal_count: int,
        ...
        summary = f"血常规检查发现 {abnormal_count} 项异常指标：{', '.join(abnormal_names)}。"
